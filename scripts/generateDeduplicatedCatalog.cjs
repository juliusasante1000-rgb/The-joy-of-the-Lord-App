...
// Raw distinct biblical places library - Each with distinct name, subtitle, icon, spiritual meaning, description, biblical reference, themes, gradient, badge
const rawUniqueBiblicalPlaces = [
  // Patriarchal & Early Covenant
  {
    name: "Rehoboth",
    subtitle: "A Place of Divine Room & Fruitfulness",
    icon: "🕊️",
    spiritualMeaning: "The turning point where contention ceases and God creates spacious room for your life and calling to flourish.",
  ...
  {
    name: "Esek",
    subtitle: "Overcoming Contention & Hostility",
    icon: "⚡",
    spiritualMeaning: "The place where the enemy disputes your initial breakthrough, forging patient endurance and unshakeable trust in God's unending wellspring.",
    ...
    badgeText: "Contention Overcome",
    colorGradient: "from-orange-700 via-rose-900 to-slate-900",
    historicalContext: "A contested valley well in the Philistine borderlands of Gerar."
  },
  {
    name: "Sitnah",
    subtitle: "Triumphing Over Accusation & Enmity",
    icon: "🛡️",
    spiritualMeaning: "Refusing to engage in toxic bitterness when faced with malice, choosing instead to move forward toward God's higher calling.",
    description: "They dug another well, and strove for that also: and he called the name of it Sitnah (Enmity). Isaac moved on without retaliation.",
    biblicalReference: "Genesis 26:21",
    themes: ["Patience", "Overcoming Accusation", "Integrity", "Focus"],
    badgeText: "Enmity Triumphed",
    colorGradient: "from-red-800 via-purple-950 to-slate-900",
    historicalContext: "A second contested well in the Negev, named from the root meaning 'accusation' or 'adversary'."
  },
  {
    name: "Ebenezer",
    subtitle: "The Stone of Divine Help & Remembrance",
    icon: "🪨",
    spiritualMeaning: "A consecrated monument declaring that through every storm, delay, and battle, the Lord has faithfully sustained us.",
    ...
    historicalContext: "A battlefield memorial between Mizpah and Shen following national deliverance from the Philistines."
  },
  {
    name: "Bethel",
  ...
  },
  {
    ...
    icon: "✨",
    spiritualMeaning: "The holy wrestling ground where human striving yields, face-to-face intimacy occurs, and identity is permanently transformed.",
  ...
  {
    name: "Mizpah",
    subtitle: "The Watchtower of Covenant Boundaries",
    icon: "👁️",
  ...
  {
    name: "Jehovah Jireh (Mount Moriah)",
  ...
  },
  {
    ...
    historicalContext: "Gideon's hometown in Manasseh."
  },
  {
    name: "Bochim",
  ...
  {
    name: "En-Gedi (Rocks of the Wild Goats)",
  ...
  {
    name: "Mount of Olives (Ascension Peak)",
  ...
  {
    name: "Sea of Galilee (Tabgha / Shore of Fire)",
  ...
  },
  {
    ...
    colorGradient: "from-purple-900 via-amber-950 to-slate-900",
    historicalContext: "Traditionally identified with Mount Tabor in Galilee or Mount Hermon."
  },
  {
  ...
  },
  {
    ...
    historicalContext: "Near ancient Shechem at the foot of Mount Gerizim and Mount Ebal."
  },
  {
    name: "Beautiful Gate of the Temple",
  ...
  {
    name: "Street Called Straight (Damascus)",
  ...
  {
    name: "House of Cornelius (Caesarea Maritima)",
  ...
  {
    name: "Lecture Hall of Tyrannus (Ephesus)",
  ...
  {
    name: "Heavenly Throne Room (Sea of Glass)",
...
console.log(`Building deduplicated spiritual places catalog with ${rawUniqueBiblicalPlaces.length} foundational canonical places...`);

// Let's generate a verified catalog of UNIQUE places, ensuring NO DUPLICATE NAMES and NO PILLARS.
...
for (const p of rawUniqueBiblicalPlaces) {
  const normName = p.name.trim().toLowerCase();
  const id = p.name.toLowerCase().replace(/[^a-z0-9]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
  
  if (!registeredNames.has(normName) && !registeredIds.has(id)) {
...
// Write spiritualPlaces500Catalog.ts with pure unique places (no pillars, no duplicate names)
const fileContent = `import { SpiritualPlace } from "../types";

// Master Verified Catalog of Unique Biblical Spiritual Places (Every place is distinct with NO duplicate names and NO pillars)
export const SCRIPTURAL_PLACES_500_CATALOG: SpiritualPlace[] = ${JSON.stringify(finalPlaces, null, 2)};

export function generate500PlusScripturalPlaces(): SpiritualPlace[] {
  // Returns clean deduplicated catalog with unique places
...
console.log(`Successfully generated src/data/spiritualPlaces500Catalog.ts with ${finalPlaces.length} unique biblical places!`);
